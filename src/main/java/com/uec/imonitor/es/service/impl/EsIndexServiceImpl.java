...
public class EsIndexServiceImpl implements IEsIndexService {
	...
	/**
	 * <p>Description:索引所有新闻</p>
	 * <p>Author:jlchen/陈金梁</p>
	 * @throws Exception 
	 * @throws BaseException 
	 * @Title: initAllNews
	 * @see com.uec.inews.es.service.IEsIndexService#initAllNews()
	 */
	@Override
...
//		Integer min = 163768;
		...
		for (int i = 0; i < t; i++) {
			...
			try {
				// 新闻需求详情索引
				 List<RequestNewsDetail> RequestNewsDetailList =
						 requestNewsService.listDetailByIds(statusList);
				 dataIndex.bulkUpdate(requestNewsName, requestNewsType, RequestNewsDetailList);
				 logger.info(CommonUtil.toJson(statusList));

			} catch (Exception e) {
				Date d = new Date();
				logger.error("执行插入需求新闻内容失败" + ", 时间=" + d
				 + ", webpageIds =" + CommonUtil.toJson(statusList) + "," + e);
				return false;
...
//		Integer min = 163768;
		...
		for (int i = 0; i < t; i++) {
			...
			try {
				// 新闻需求详情索引
				 List<NewsSpreadingAnalysisDetail> newsSpreadingAnalysisList =
						 newsSpreadingAnalysisService.listNewsSpreadingAnalysisDetailByIds(statusList);
